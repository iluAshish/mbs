...
.media_wrapper img:hover {opacity: 0.7;}
...
@media only screen and (max-width: 700px){
    ...
    <div class="content-wrapper">
        ...
        <section class="content media_wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        @if (session('success'))
                        ...
                        @endif
                            ...
                            <div class="gloss_card-header">
                                <a href="{{url(Helper::sitePrefix().'media/create')}}"
                                   class="btn btn-success pull-right">Add media <i
                                ...
                                <table id="recordsListView" class="table table-bordered table-hover dataTable">
                                    ...
                                    @foreach($medias as $media)
                                        <tr>
                                            <td>{{ $i }}</td>
                                            <td>{{ $media->title }}</td>
                                            {{-- <td><img width="100" src="{{url('/')}}/{{ $media->image_webp }}" alt="" srcset=""></td> --}}
                                            <td><a href="{{url(Helper::sitePrefix().'media/gallery/'.$media->id)}}"
                                                class="btn btn-sm btn-primary mr-2 tooltips" title="Add Gallery">Gallery</a>
                                            </td>
                                            <td>
                                                <input type="text" name="sort_order"
                                                       id="gallery_order_{{$loop->iteration}}"
                                                       data-table="Media" data-id="{{ $media->id }}"
                                                       data-field="id"
                                                       data-field-value="{{ $media->id }}"
                                            ...
                                            <td>
                                                <label class="switch">
                                                    <input id="switch-state" type="checkbox" class="status_check"
                                                           ...
                                                           data-table="Media"
                                                           data-field="status" data-pk="{{ $media->id }}"
                                            ...
                                            <td class="text-right py-0 align-middle">
                                                <div class="btn-group btn-group-sm">
                                                    <a href="{{url(Helper::sitePrefix().'media/edit/'.$media->id)}}"
                                                       class="btn btn-success mr-2 tooltips" title="Edit media"><i
                                                            class="fas fa-edit"></i></a>
                                                    <a href="#" class="btn btn-danger mr-2 delete_entry tooltips"
                                                       title="Delete media" data-url="media/delete"
                                                       data-id="{{$media->id}}"><i class="fas fa-trash"></i></a>
                                                </div>
                                            </td>
                                        </tr>
                                        @php $i++@endphp
...
$(document).ready(function(){
  ...
  $(".media_wrapper img").click(function(){
