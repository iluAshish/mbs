@if($services->isNotEmpty())
    @foreach($services as $service)
        <div class="product-card">
            <picture>
                <img src="{{asset($service->image_webp ?? $service->image)}}" width="245px" height="161" {{$service->image_attribute}}>
            </picture>
            <div class="product-details">
                <h3>{{$service->title ?? $service->sub_title}}</h3>
                
                <p>{{ \Illuminate\Support\Str::words(strip_tags($service->short_description), 20, '...') }}</p>
            </div>
          
            <a href="{{url('service-detail', ['short_url' => $service->short_url]) }}" class="btn product-btn">
                View More
